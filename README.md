...
- ajoute un widget de tableau de bord **« Prochain repas »**.
...
Le point le plus risqué du POC est de savoir si Mealie et Bonap démarrent **sans aucune capability Linux**, comme Gladys lance ses sous-conteneurs. `docker-compose.sandbox.yml` reproduit ces contraintes (`cap_drop: ALL`, `no-new-privileges`, 100 pids, `/tmp` noexec).
...
3. Les packages ghcr doivent être **publics** (Package settings → Change visibility).
